...
let usernames = {}; // placeholder to use when updating user list
let usernames1 = {};
let usernames2 = {};
let usernames3 = {};
...
const main = io.on('connection', (socket) => {
    ...
    socket.on('new_client', (username) => {
        ...
        if (username === 'ALL') {
            socket.emit('duplicate_username', "ALL is an reserved word. Please enter another one.")
        } else if (findValue(user_main, username)) {
            ...
            if (!user_main.hasOwnProperty(socket.id)) {
                ...
                io.sockets.emit('online_users', user_main);
    ...
    socket.on('chat_message', (message) => {
        ...
        const regExp = /^[@].*:/;
        let isPrivate = regExp.test(message);
        
        if (isPrivate) { // msg starts with @Name
            let privateString = message.split(':');
            let receiveUser = privateString[0].substr(1);
            console.log(receiveUser);
            // @ALL -> send msg to all online users
            if (receiveUser === 'ALL') {
                socket.emit('chat_message', { username: "Me", message: message });
                socket.broadcast.emit('chat_message', { username: socket.username, message: message });
            } else { // send to a specific user
                for (let key in user_main) {
                    if (user_main[key] === receiveUser) {
                        let receiveId = key;
                        socket.emit('chat_message', { username: "Me", message: message });
                        socket.broadcast.to(receiveId).emit('chat_message', { username: socket.username, message: message, type: "private" });
                    }
                }
            }
        } else { // msg to each room
            socket.emit('chat_message', { username: "Me", message: message });
...
/*
    // create new room
    socket.on('new_room', () => {
        let newroom = `room${rooms.length + 1}`;
        console.log(newroom);
        rooms.push(newroom);
        io.sockets.emit('create_room', newroom);
    });
*/
